...
def emergency_lockdown_message(policy=None):
    policy = policy or SecurityPolicy.get_solo()
...
def lockdown_exempt_user(user, policy=None):
    ...
    if not user or not getattr(user, "is_authenticated", False):
        return False
    return is_admin(user) or user.username.lower() in policy.break_glass_accounts


def lockdown_applies_to_user(user, policy=None):
    policy = policy or SecurityPolicy.get_solo()
    return emergency_lockdown_active(policy=policy) and not lockdown_exempt_user(user, policy=policy)
